...
1. *The 1-WL barrier is crossed by particles, not by interference.* A GNN, like a coherent field, aggregates messages that have forgotten where they came from; the Born rule with a definite source, $\sum_s |M_{vs}|^2$, computes a *closed* walk, and closed walks are exactly what colour refinement cannot count (Theorem 1). Any source-resolved intensity has this, classical or quantum.
...
On the left, two walks $s \to v$ and $s' \to v$ are glued at $v$ only and their sources are free: the pattern is a path, a tree, and the count of tree patterns is what 1-WL determines (Dvořák; Dell–Grohe–Rattan). On the right the two walks share both endpoints: the pattern is a cycle, whose count 1-WL does not determine — the triangle count $\operatorname{tr} A^3$ is the first example, and $C_6$ versus $2C_3$ the first pair. A GNN is on the left: its aggregation is a function of the multiset of incoming messages, so a message that has come back to its origin is treated like any other, and the node state is a function of the unfolding tree (Xu et al. 2019; Morris et al. 2019). A particle is on the right: the detector asks, of the light arriving at $v$, how much started at each $u$ — including at $v$ itself.

Nothing quantum is needed for the right-hand side. It is the intensity at $v$ of mutually incoherent sources (handout §5.2, ii), and it is also what one gets by running the interferometer once per source with a laser and adding the recorded intensities. In GNN terms it is the return probability of a random walk, the diagonal of $P^k$, which is precisely the *random-walk structural encoding* that lifts message passing above 1-WL at linear cost (Dwivedi et al. 2022), as subgraph counts do (Bouritsas et al. 2022). So "beyond 1-WL yet classically simulable" is no paradox: the 1-WL bound of GNNs is a bound on *anonymity* — messages carry no identity — not on computational power, and every construction that lets a node recognise its own returning signal escapes it, at polynomial cost. The passive photonic model escapes it because a photon has a source. Distinguishable photons, indistinguishable photons and classical incoherent light all do.
...
- V. P. Dwivedi, A. T. Luu, T. Laurent, Y. Bengio, X. Bresson, *Graph neural networks with learnable structural and positional representations* (ICLR 2022) — random-walk return probabilities as node features; G. Bouritsas, F. Frasca, S. Zafeiriou, M. Bronstein, *Improving graph neural network expressivity via subgraph isomorphism counting* (TPAMI 2022).
