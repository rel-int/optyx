...
**Remark (the mark in the bosonic setting).** The photons of the homogeneous drive are injected together, one per drive per tick, and are indistinguishable, so no photon carries a source. The *mode* does: every slot is a distinct optical mode holding exactly one photon, and a Fock state has no phase coherence between modes, $\langle b_s^\dagger b_{s'} \rangle = 0$ for $s \ne s'$. That absence of inter-source coherence is the mark, and photons of the same tick, in the drives of different nodes, carry it as much as photons of different ticks. A single detector reads it: $\langle c_v^\dagger c_v \rangle = \sum_{s, s'} \overline{M_{vs}} M_{vs'} \langle b_s^\dagger b_{s'} \rangle = \sum_s p_s(v)$, and likewise every higher moment of one mode, which is Theorem 2. Two detectors partly erase it: the fourth-order correlator of a Fock input weighs equally the assignment "$s$ at $v$, $s'$ at $w$" and the assignment "$s'$ at $v$, $s$ at $w$", and their interference is the exchange term (4.1). For distinguishable photons the internal state tells the two assignments apart, so they add in probability and the mark is readable everywhere; for a coherent drive there is inter-source coherence, $\langle b_s^\dagger b_{s'} \rangle = \overline{\alpha_s} \alpha_{s'}$, so there is no mark even at one detector, which is Theorem 1(a). In the language of §2 the bosonic model is a node-marking subgraph GNN with one cross-mark layer: $x_{ss'}$ is the inner product of the node features of the $s$-marked and the $s'$-marked runs on the output nodes, and $X$ the off-diagonal Frobenius norm of their Gram matrix. Cross-subgraph layers keep a node-marking subgraph GNN within 3-WL (Frasca et al. 2022), as Theorem 4 keeps $X$ within 2-FWL.
