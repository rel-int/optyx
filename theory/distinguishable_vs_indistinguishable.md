...
**Remark (the mark in the bosonic setting).** The photons of the homogeneous drive are injected together, one per drive per tick, and are indistinguishable, so no photon carries a source. The *mode* does, in the following sense. A detector at $v$ sees the input only through its first-order coherence matrix in the source basis,
$$\langle n_v \rangle = \sum_{s, s'} \overline{M_{vs}}\, M_{vs'}\, G_{ss'}, \qquad G_{ss'} := \langle b_s^\dagger b_{s'} \rangle,$$
and the source is readable at $v$ exactly to the extent that $G$ is diagonal. For a Fock drive with one photon per slot, $G = I = \sum_s e_s e_s^{\top}$: a number state has no phase relation between different modes, so each source enters as its own one-hot term and $\langle n_v \rangle = \sum_s p_s(v)$. For a coherent drive of equal amplitude in every slot, $G \propto J$, the all-ones matrix: every pair of sources is phase-coherent, the input is one run marked by the all-ones vector, and $\langle n_v \rangle \propto |\sum_s M_{vs}|^2$. The two matrices are the two generators $I$ and $J$ of the coherent algebra, the first contracting a walk's source with its conjugate's into a cycle, the second leaving both free in a tree. Physically, the Fock state is invariant under an independent phase rotation $b_s \mapsto e^{i\theta_s} b_s$ of every source, so no count on a single mode can depend on the relative phases of the sources and the only quadratic invariants are the $|M_{vs}|^2$; a coherent state is not, and $|\sum_s M_{vs}|^2$ depends on them. Independent thermal sources have the same diagonal $G$, which is why classical incoherent light reads the mark too, and photons of the same tick, in the drives of different nodes, carry it as much as photons of different ticks. Every higher moment of one mode reads the mark the same way, which is Theorem 2. Two detectors partly erase it: the fourth-order correlator of a Fock input weighs equally the assignment "$s$ at $v$, $s'$ at $w$" and the assignment "$s'$ at $v$, $s$ at $w$", and their interference is the exchange term (4.1): the source label is a property of the input mode, not of the photon, and the photon does not carry it to the detector. For distinguishable photons the internal state travels with the photon, so the two assignments are orthogonal at the detectors as well, they add in probability and the mark is readable everywhere — distinguishability makes the mark a property of the particle where a Fock drive makes it a property of the mode only; for a coherent drive there is inter-source coherence, $\langle b_s^\dagger b_{s'} \rangle = \overline{\alpha_s} \alpha_{s'}$, so there is no mark even at one detector, which is Theorem 1(a). In the language of §2 the bosonic model is a node-marking subgraph GNN with one cross-mark layer: $x_{ss'}$ is the inner product of the node features of the $s$-marked and the $s'$-marked runs on the output nodes, and $X$ the off-diagonal Frobenius norm of their Gram matrix. Cross-subgraph layers keep a node-marking subgraph GNN within 3-WL (Frasca et al. 2022), as Theorem 4 keeps $X$ within 2-FWL.
