...
:func:`generate` from a ``builder-inited`` hook, so ``sphinx-build`` renders
the notebooks as part of a normal docs build. If a notebook cannot be executed
...
It can also be run as a script::
    ...
    python docs/export_notebooks.py bose_hubbard  # render a single notebook
...
# Notebooks with a pre-existing failure unrelated to the marimo conversion,
# each tracked by an issue so ``--check`` does not block on it; ``generate``
# already falls back to a link page for any notebook that fails to execute.
KNOWN_BROKEN = {
    "compile_to_semm": "https://github.com/rel-int/optyx/issues/63",
    "feed_forward_example": "https://github.com/rel-int/optyx/issues/62",
    "photon_distinguishability": "https://github.com/rel-int/optyx/issues/62",
    "readme_example": "https://github.com/rel-int/optyx/issues/62",
}
...
.. note::
    ...
    `{name}.md
    <https://github.com/rel-int/optyx/blob/main/docs/notebooks/{name}.md>`_.
"""
...
def main() -> int:
    ...
    for notebook in selected:
        ...
        try:
            ...
            if args.check and notebook.stem in KNOWN_BROKEN:
                print(f"  known failure, see {KNOWN_BROKEN[notebook.stem]}",
                      file=sys.stderr)
                continue
        ...
        if not args.check:
            write_page(notebook, rendered=True)
    if failures:
        print(f"failed to execute: {', '.join(failures)}", file=sys.stderr)
        return 1
    return 0
