...
- [WIP] @session_01BVf5U3AGWKwGHPMNh2SxvR-2026-09-07 11:43 `experiments/invariant_qmapnn/cells.py`: `invariant_cell(d, W, psi)` (§3), the contrast cells, and the functor from a graph with a rotation system to an `optyx.interaction.CMap` in the dart-pair / memory / drive / output layout (§2)
- [ ] `experiments/invariant_qmapnn/engine.py`: the step matrix assembled from the functor image, checked against `CMap.step.to_path()`; the spacetime transfer over injection slots; the two-photon, one-photon and coherent-state certificates read on the final output modes (§5, §6, H4)
- [ ] `experiments/invariant_qmapnn/run.py` + `report.py`: one resumable CSV row per (pair, cell, T), regimes R1 (canonical, Grover point) and R2 (20 seeds), pruning as in PR #69, the V2 rotation table and the V3 relabelling rows (§4, §7)
- [ ] `test/test_invariant_qmapnn.py`: V1 Schur lemma, assembled == `CMap.step.to_path()`, the triangle and path functor-image grounding, V2 on P4 for both cells, V3 on P4, V4 (§7, §9)
- [ ] Run controls, the 1-FWL rung at T = 2, 3, 4, the rook/Shrikhande sanity row at T = 2, the rotation and relabelling tables, H4; commit `results/`
